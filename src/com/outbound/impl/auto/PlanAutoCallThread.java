...
public class PlanAutoCallThread extends Thread {
	private Logger logger = Logger.getLogger(PlanAutoCallThread.class.getName());
	...
	// private String uui =
	// "name_gender_loanName_loanAmount_dueDate_days_idnum_wages_work1_wok2";
	...
	private List<RosterBatchInfo> batchInfos = new ArrayList<RosterBatchInfo>();

	// 是否获取到了批次信息

	public PlanAutoCallThread(String type, ActivityInfo _activity, OutboundPolicyInfo _policyInfo) {
		taskType = type;
	...
	public void getUnCallRosterBatchs(){
		List<RosterBatchInfo> newBatchs = batchDao.findUncallBatch(domain, getActivityInfo().getRosterTemplateName());
		logger.info("## " + this.getName() +" get uncallroster batchs "+ newBatchs);
		if(newBatchs != null){
			batchInfos.addAll(newBatchs);
		}
	}
	...
	public void run() {
		...
		logger.info("##### " + taskType + " taskthread [" + getActivityInfo().getName() + ":" + getActivityInfo().getDomain()
				+ "] start to run ");
		TaskContainer.planAutoCallActivityMap.put(activityName, this);
		initParams();
		if(status == 0){
			status = 1;	
		}
		while(status != 3){
			...
			while(status == 1){
				...
				for(RosterBatchInfo batchInfo: batchInfos){
					if(batchInfo != null){
		...
		logger.info("---###### activity " + getActivityInfo() + " excute complete " + getActivityInfo() + " costtime "
				+ costtime / 1000 + " s");
		...
		TaskContainer.deleteActivityThread(activityName);
		excuteCallBack();
	...
	private void checkActivityTime(){
		while(!isActivityEffective() && (status == 1)){
			try {
				Thread.sleep(5000);
			} catch (InterruptedException e) {
				logger.error(e);
	...
	private void checkPolicyTime(){
		while(!isPolicyEffective() && (status == 1)){
			try {
	...
	private boolean isEffective(Date bTime, Date eTime) {
		...
		if (bTime.getTime() > cTime.getTime()){
			logger.info("## isPolicyEffective domain:" +activityInfo.getDomain() + 
					" activity: " + activityInfo.getName() + 
					" begintime:" + activityInfo.getBeginDatetime() + 
					"currentTime:" + cTime);
			return false;
